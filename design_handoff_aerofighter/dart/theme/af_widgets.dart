...
/// `active` flips to the orange selected treatment.
        ...
        child: GestureDetector(
          ...
          child: Container(
            ...
            decoration: BoxDecoration(
              ...
              color: active ? null : AF.surface.withOpacity(0.6),
...
/// Floating bottom tab bar (HOME / HANGAR / SHOP / PILOT). The selected tile
/// fills orange + glows; the rest are cyan glyph+label.
class AfNavBar extends StatelessWidget {
  ...
  Widget build(BuildContext context) => Container(
        ...
        decoration: BoxDecoration(
          color: AF.surface.withOpacity(0.95),
...
/// One option in [AfSegmented]. `color` overrides the active tint (e.g. green
/// EASY, red HARD).
class AfSegment {
  final String label;
  final Color? color;
...
/// Segmented selector — the DIFFICULTY SCALE (EASY / NORMAL / HARD). The
/// selected option tints with its color at low opacity + a colored border.
  ...
  Widget _seg(AfSegment opt, bool selected, VoidCallback onTap) {
    ...
    return GestureDetector(
      ...
      child: AnimatedContainer(
        ...
        decoration: BoxDecoration(
          color: selected ? col.withOpacity(0.2) : Colors.black.withOpacity(0.22),
          borderRadius: BorderRadius.circular(AF.rSm),
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          child: Container(width: 22, height: 22, decoration: BoxDecoration(
            shape: BoxShape.circle, color: value ? AF.bg : Colors.white.withOpacity(0.5))),
        ),
      );
}

/// Hangar jet-selector chip — a glowing skin-color dot in a rounded tile.
/// `highlighted` (being viewed) → 2px cyan border + tint; `equipped` → green check.
class AfJetChip extends StatelessWidget {
  ...
  Widget build(BuildContext context) => GestureDetector(
        ...
        child: AnimatedContainer(
          ...
          decoration: BoxDecoration(
            color: highlighted ? AF.cyanMid.withOpacity(0.10) : Colors.black.withOpacity(0.25),
            borderRadius: BorderRadius.circular(AF.rMd),
            border: Border.all(
              color: highlighted ? AF.cyan : (equipped ? AF.cyanMid.withOpacity(0.5) : AF.borderInactive),
...
/// Glowing ringed power-up badge for menu/HUD widget surfaces (daily rewards,
/// pickup toasts). The in-world pickup itself is a Flame sprite from
/// `powerups_strip_128.png`; this is the Flutter-widget representation.
      ...
      decoration: ring
          ? BoxDecoration(
              ...
              gradient: RadialGradient(
                colors: [meta.color.withOpacity(0.28), meta.color.withOpacity(0.05), Colors.transparent],
                stops: const [0.0, 0.7, 1.0]),
              border: Border.all(color: meta.color.withOpacity(0.6), width: AF.stroke),
