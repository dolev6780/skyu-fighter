...
| `af_widgets.dart` | Drop-in widgets — all 14 DS components: `AfButton`, `AfIconButton`, `AfPanel`, `AfGlassModal`, `AfStatBar`, `AfProgressBar`, `AfBadge`, `AfJetChip`, `AfWorldChip`, `AfNavBar`, `AfSegmented`, `AfToggle`, `AfAvatar`, `AfPowerUpBadge`. |

### `game/` — ready-wired game source
Drop-in refactor of the actual game screens onto this port. See
`game/INTEGRATION.md` for the 3-step install + the `main.dart` patch.
| File | Replaces |
|---|---|
| `game/start_menu.dart` | `lib/start_menu.dart` (HOME / HANGAR / SHOP / PILOT) |
| `game/game_overlays.dart` | the overlay classes in `lib/main.dart` (Game Over, Mission Complete, Paused, pause button) |
